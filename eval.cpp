...
int eval::evaluate() {
    ...
    for (int i = 0; i < 144; i++) {
        ...
        midgame[pieceColor] += midValues[pieceNumber % 6];
        endgame[pieceColor] += endValues[pieceNumber % 6];

        gamePhase += gamephaseInc[pieceNumber];
    }

    /* tapered eval */
    int midgameScore = midgame[0] - midgame[1];
    int endgameScore = endgame[0] - endgame[1];
