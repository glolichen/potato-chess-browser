...
namespace PieceMoves {
    export function pmoves(loc: number): Moves.Move[] {
        ...
        let offset: number = loc + Pieces.Offsets.PAWN_MOVE[Board.turn ? 1 : 0][0];
        ...
        for (let offset2 of Pieces.Offsets.PAWN_CAPTURE[Board.turn ? 1 : 0]) {
            let i: number = loc + offset2;
            if (!Board.board[i] || Board.board[i] == -1)
                continue;

            let turn: boolean = Board.turn;
            let pieceColor: boolean = Pieces.getColor(Board.board[i]);
    ...
    export function nmoves(loc: number): Moves.Move[] {
        let moves: Moves.Move[] = [];
    ...
    export function bmoves(loc: number): Moves.Move[] {
        let moves: Moves.Move[] = [];
    ...
    export function rmoves(loc: number, isRook: boolean): Moves.Move[] {
        let moves: Moves.Move[] = [];
    ...
    export function kmoves(loc: number): Moves.Move[] {
        let moves: Moves.Move[] = [];
        ...
        if (Board.turn == Constants.WHITE) {
            let KLegal: boolean = true;
            let QLegal: boolean = true;
            for (let c of Attacked.attacked) {
                if (c == 115 || c == 116)
                    KLegal = false;
                if (c == 113 || c == 112)
