...
ull maps::fill[64][64];
std::unordered_map<int, ull> maps::pinnedOffsets[64];
...
void maps::init() {
	for (int i = 0; i < 64; i++) {
		...
		for (int j = 1; j <= 7; j++) {
			maps::pinnedOffsets[i].insert({ j * -8, maps::rook[i][0] });
			maps::pinnedOffsets[i].insert({ j * 8, maps::rook[i][2] });
			maps::pinnedOffsets[i].insert({ j * 1, maps::rook[i][1] });
			maps::pinnedOffsets[i].insert({ j * -1, maps::rook[i][3] });
			maps::pinnedOffsets[i].insert({ j * -7, maps::bishop[i][0] });
			maps::pinnedOffsets[i].insert({ j * 7, maps::bishop[i][2] });
			maps::pinnedOffsets[i].insert({ j * 9, maps::bishop[i][1] });
			maps::pinnedOffsets[i].insert({ j * -9, maps::bishop[i][3] });
