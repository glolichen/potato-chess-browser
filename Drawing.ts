...
var moves: Moves.Move[] = moveGen();
var movesFromSelected: Moves.Move[] = [];

document.getElementById("perft")!.onclick = () => window.open("./perft.html");

function moveGen(): Moves.Move[] {
    let moves: Moves.Move[] = MoveGen.moveGen();
    if (moves.length == 0) {
        Attacked.getAttacked();

        let inCheck: boolean = Attacked.attacked.has(Board.king[Board.turn ? 1 : 0]);
...
function pickPromotion(square: number) {
    paused = true;
